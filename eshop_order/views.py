...
def add_user_order(request):
    ...
    if new_order_form.is_valid():
        ...
        if count < 0:
            count = 1
        product = Product.objects.get_product_id(product_id=product_id)
...
def user_open_order(request, *args, **kwargs):
    ...
    if open_order is not None:
        ...
        context['tax'] = open_order.tax()
        context['final_price'] = open_order.get_final_amount()
...
def verify(request, *args, **kwargs):
    ...
    if request.GET.get('Status') == 'OK':
        ...
        if result.Status == 100:
            user_order = Order.objects.get_queryset().get(order_id)
